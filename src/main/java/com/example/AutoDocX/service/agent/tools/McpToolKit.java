...
public class McpToolKit {
    ...
    private List<Tool> buildTools(List<String> names) {
        List<FunctionDeclaration> decls = allTools.stream().filter(tool -> names.contains(tool.name().orElse(""))).collect(Collectors.toList());
    ...
    public String executeTool(String toolName, Map<String, Object> params, ToolExecutionContext context) {
        ...
        try {
            ...
            switch (toolName) {
                case "read_file":
                case "get_code":
                    context.getSession().getMemory().getCode().addEntry(extractIDFromParams(params), result);
                    break;
                ...
                case "get_modified_nodes":
                    ...
                    break;
                case "summarise_code":
                    context.getSession().getMemory().getSummary().addEntry(extractIDFromParams(params), result);
                    context.getSession().getMemory().getCode().removeEntry(extractIDFromParams(params));
                    break;
                ...
                case "update_understanding":
                    ...
                    context.getEpisodicMemory().addEntry("model", "Tool call: " + toolName + "{" + "..." + "}" + ", Result: Success");
                    return result;
                default:
                    System.out.println("WARNING: unknown tool: " + toolName + paramForMemory);
                    break;
            }
            context.getEpisodicMemory().addEntry("model", "Tool call: " + toolName + paramForMemory + ", Result: Success");
        } catch (Exception e) {
            result = e.getClass().getSimpleName() + ": " + e.getMessage();
            System.err.println("DEBUG: Tool execution failed for " + toolName + paramForMemory + ", Error: " + e.getMessage());
...
//                int countdown = ((Number) paramsMap.get("countdown")).intValue();
            ...
            case "get_modified_nodes": {
                ...
                String result = mcpToolUtils.getModifiedNodes(context.getGraph(), context.getSession().getDocumentationHandler(), docKey, mcpToolUtils.getGitService(), context.getRepo().getClonedPath());
                context.getSession().getMemory().getSummary().addEntry(toolName + "{" + docKey + "}", result);
                return result;
            }
            default:
    ...
    @SuppressWarnings("unchecked")
    private String extractIDFromParams(Object param) {
